...
class ListCombinations
{
    ...
    enum Symbol
    {
        Mrn, ResourceId, DeptId, ApptId, CheckInStatus
    ...
    static void combinations(int locNumber)
    {
        ...
        for (int valueNumber = 0; valueNumber < filterValues.length; valueNumber++)
        {
            positions[locNumber] = filterValues[valueNumber];
            if (locNumber < (filterValues.length - 1))
            {
            ...
            if (locNumber == (filterValues.length - 1) && repeatedCards(positions))
            {
